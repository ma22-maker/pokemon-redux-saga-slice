...
//         const types = [];
//         item.types.map((type) => types.push(type.type.name));
...
const readucerdata = createSlice({
  name: "AnimeData",
  initialState,
  reducers: {
    listingData: (state) => {
    ...
    listingData_SUCCESS: (state, action) => {
      state.loading = false;
      state.error = null;
      const matter = [];
      action.payload.animeData.map((item) => {
        const types = [];
        item.types.map((type) => types.push(type.type.name));
        //console.log(types,"types")
        const anime = {
    ...
    detailsPage_SUCCESS:(state,action)=>{
        state.loading = false;
        state.error = null;
        //console.log(action.payload.two)
...
export const { listingData, listingData_SUCCESS, detailsPage, detailsPage_SUCCESS, Data_FAILURE } = readucerdata.actions;


const store = configureStore({
    reducer: {
        AnimeData: readucerdata.reducer,
      },
...
console.log(readucerdata.actions)
